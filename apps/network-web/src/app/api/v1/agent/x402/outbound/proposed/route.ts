...
type AgentX402OutboundProposedRequest = {
  schemaVersion: 1;
  paymentId: string;
  approvalId: string;
  networkKey: string;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validated = validatePayload<AgentX402OutboundProposedRequest>('agent-x402-outbound-proposed-request.schema.json', parsed.body);
    if (!validated.ok) {
      return errorResponse(
        400,
        {
          code: 'payload_invalid',
          message: 'x402 outbound proposed payload does not match schema.',
          actionHint: 'Provide canonical x402 outbound proposed fields.',
