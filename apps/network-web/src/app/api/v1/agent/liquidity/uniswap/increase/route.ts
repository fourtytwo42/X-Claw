...
import { increaseLpUniswap, isUniswapLpEligibleChain, UniswapProxyError } from '@/lib/uniswap-lp-proxy';
import { validatePayload } from '@/lib/validation';

type UniswapLpIncreaseRequest = {
  agentId: string;
  chainKey: string;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validated = validatePayload<UniswapLpIncreaseRequest>('uniswap-lp-increase-request.schema.json', parsed.body);
    if (!validated.ok) {
      return errorResponse(
        400,
        {
          code: 'payload_invalid',
          message: 'Uniswap LP increase payload does not match schema.',
    ...
    const result = await increaseLpUniswap(body);
