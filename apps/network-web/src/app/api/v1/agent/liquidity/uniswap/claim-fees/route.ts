...
import { claimLpFeesUniswap, isUniswapLpEligibleChain, UniswapProxyError } from '@/lib/uniswap-lp-proxy';
import { validatePayload } from '@/lib/validation';

type UniswapLpClaimFeesRequest = {
  agentId: string;
  chainKey: string;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validated = validatePayload<UniswapLpClaimFeesRequest>('uniswap-lp-claim-fees-request.schema.json', parsed.body);
    if (!validated.ok) {
      return errorResponse(
        400,
        {
          code: 'payload_invalid',
          message: 'Uniswap LP claim-fees payload does not match schema.',
    ...
    const result = await claimLpFeesUniswap(body);
