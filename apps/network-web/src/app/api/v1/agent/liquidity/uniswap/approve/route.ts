...
import { approveLpUniswap, isUniswapLpEligibleChain, UniswapProxyError } from '@/lib/uniswap-lp-proxy';
import { validatePayload } from '@/lib/validation';

type UniswapLpApproveRequest = {
  agentId: string;
  chainKey: string;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validated = validatePayload<UniswapLpApproveRequest>('uniswap-lp-approve-request.schema.json', parsed.body);
    if (!validated.ok) {
      return errorResponse(
        400,
        {
          code: 'payload_invalid',
          message: 'Uniswap LP approve payload does not match schema.',
    ...
    const result = await approveLpUniswap(body);
