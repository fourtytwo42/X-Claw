...
import { createLpUniswap, isUniswapLpEligibleChain, UniswapProxyError } from '@/lib/uniswap-lp-proxy';
import { validatePayload } from '@/lib/validation';

type UniswapLpCreateRequest = {
  agentId: string;
  chainKey: string;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validated = validatePayload<UniswapLpCreateRequest>('uniswap-lp-create-request.schema.json', parsed.body);
    if (!validated.ok) {
      return errorResponse(
        400,
        {
          code: 'payload_invalid',
          message: 'Uniswap LP create payload does not match schema.',
    ...
    const result = await createLpUniswap(body);
