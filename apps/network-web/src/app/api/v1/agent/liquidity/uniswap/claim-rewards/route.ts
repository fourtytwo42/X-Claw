...
import { claimLpRewardsUniswap, isUniswapLpOperationEnabled, UniswapProxyError } from '@/lib/uniswap-lp-proxy';
import { validatePayload } from '@/lib/validation';

type UniswapLpClaimRewardsRequest = {
  agentId: string;
  chainKey: string;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validated = validatePayload<UniswapLpClaimRewardsRequest>(
    ...
    if (!validated.ok) {
      return errorResponse(
        400,
        {
          ...
          message: 'Uniswap LP claim-rewards payload does not match schema.',
    ...
    if (!isUniswapLpOperationEnabled(body.chainKey, 'claim_rewards')) {
      return errorResponse(
        400,
        {
    ...
    const result = await claimLpRewardsUniswap(body);
