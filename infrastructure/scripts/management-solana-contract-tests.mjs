...
function anyTextIncludes(paths, needle) {
  return paths.some((relPath) => readText(relPath).includes(needle));
}
...
async function main() {
  ...
  expect(
    anyTextIncludes(
      ...
      'Use a valid Solana base58 address.'
    ),
    'runtime_wallet_send_solana_address_validation'
  );
  expect(
    anyTextIncludes(
      [
    ...
    'runtime_trade_spot_solana_distinct_mints'
  );
  expect(
    anyTextIncludes(
      [
  ...
  expect(
    ...
    'runtime_trade_execute_mock_rejected'
  );
