...
def main(argv: List[str]) -> int:
    ...
    if cmd == "wallet-rpc-health":
        target_chain = str(argv[2]).strip() if len(argv) >= 3 and str(argv[2]).strip() else chain
        return _run_agent(["wallet", "rpc-health", "--chain", target_chain, "--json"])
