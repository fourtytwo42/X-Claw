# Slice 234 Spec: Telegram Messaging + Delivery Cleanup Services (2026-03-08)

Issue mapping: `#87`

## Goal
1. Move Telegram and owner-link delivery helper ownership out of `cli.py` into runtime services.
2. Preserve current command contracts, delivery semantics, and existing patch/test seams.
3. Keep `cli.py` as parser/router + thin wrapper surface only for the moved helpers.

## Non-goals
1. No API route/schema/database changes.
2. No approval UX or message-text redesign.
...
3. `apps/agent-runtime/xclaw_agent/runtime/services/telegram_delivery.py`
4. `apps/agent-runtime/xclaw_agent/runtime/services/owner_link_delivery.py`
